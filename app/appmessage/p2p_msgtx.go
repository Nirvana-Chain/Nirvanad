...
// Outpoint defines a nirvana data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new nirvana transaction outpoint point with the
...
// TxIn defines a nirvana transaction input.
type TxIn struct {
...
// NewTxIn returns a new nirvana transaction input with the provided
...
// TxOut defines a nirvana transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new nirvana transaction output with the provided
...
// MsgTx implements the Message interface and represents a nirvana tx message.
