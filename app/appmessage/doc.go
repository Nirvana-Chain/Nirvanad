...
supported nirvana messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Nirvana Message Overview

The nirvana protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which nirvana network it is a part of, its type, how big it is, and a checksum
to verify validity. All encoding and decoding of message headers is handled by
this package.

To accomplish this, there is a generic interface for nirvana messages named
...
of the currently supported nirvana messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using nirvana encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the nirvana messages are intended
...
and write nirvana messages. The following sections provide a quick overview of
...
The nirvana network is a magic number which is used to identify the start of a
message and which nirvana network the message applies to. This package provides
...
As discussed in the nirvana message overview section, this package reads
and writes nirvana messages using a generic interface named Message. In
...
In order to unmarshall nirvana messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a nirvana peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, nirvanaNet)
	if err != nil {
...
In order to marshall nirvana messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a nirvana peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, nirvanaNet)
	if err != nil {
		// Log and handle the error
	}
