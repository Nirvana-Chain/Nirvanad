...
import (
	...
	"github.com/Nirvana-Chain/nirvanad/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		nirvanadMessage, err := client.Post(&protowire.NirvanadMessage{Payload: &protowire.NirvanadMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := nirvanadMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	nirvanadMessage := &protowire.NirvanadMessage{}
	err := protojson.Unmarshal([]byte(response), nirvanadMessage)
	if err != nil {
	...
	return marshalOptions.Format(nirvanadMessage)
}
