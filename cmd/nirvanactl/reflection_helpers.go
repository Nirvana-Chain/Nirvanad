...
// 2. A wrapper that implements isNirvanadMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `NirvanadMessage_` prefix and `Request` suffix
...
// generateNirvanadMessage generates a wrapped NirvanadMessage with the given `commandValue`
func generateNirvanadMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.NirvanadMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	nirvanadMessage := reflect.New(reflect.TypeOf(protowire.NirvanadMessage{}))
	nirvanadMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return nirvanadMessage.Interface().(*protowire.NirvanadMessage), nil
}
