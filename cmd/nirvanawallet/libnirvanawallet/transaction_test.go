	...
	testutils.ForAllPaths(t, func(t *testing.T, version uint32) {
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
				...
				path := "m/1/2/3"
				address, err := libnirvanawallet.Address(params, publicKeys, minimumSignatures, path, ecdsa)
				if err != nil {
				...
				isFullySigned, err := libnirvanawallet.IsTransactionFullySigned(unsignedTransaction)
				if err != nil {
				...
				_, err = libnirvanawallet.ExtractTransaction(unsignedTransaction, ecdsa)
				...
				signedTxStep1, err := libnirvanawallet.Sign(params, mnemonics[:1], unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				isFullySigned, err = libnirvanawallet.IsTransactionFullySigned(signedTxStep1)
				if err != nil {
					t.Fatalf("IsTransactionFullySigned: %+v", err)
				}
				...
				signedTxStep2, err := libnirvanawallet.Sign(params, mnemonics[1:2], signedTxStep1, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				extractedSignedTxStep2, err := libnirvanawallet.ExtractTransaction(signedTxStep2, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}

				signedTxOneStep, err := libnirvanawallet.Sign(params, mnemonics[:2], unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				extractedSignedTxOneStep, err := libnirvanawallet.ExtractTransaction(signedTxOneStep, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}
		...
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
				...
				for i := 0; i < numKeys; i++ {
					...
					publicKeys[i], err = libnirvanawallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], false, version)
					if err != nil {
						t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
					}
				}
				...
				signedTx, err := libnirvanawallet.Sign(params, mnemonics, unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				tx, err := libnirvanawallet.ExtractTransaction(signedTx, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}
...
func TestMaxSompi(t *testing.T) {
	testutils.ForAllPaths(t, func(t *testing.T, version uint32) {
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			cfg.PreDeflationaryPhaseBaseSubsidy = 20e6 * constants.SompiPerNirvana
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libnirvanawallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], false, version)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			selectedUTXOsForTxWithLargeInputAmount := []*libnirvanawallet.UTXO{
				{
					Outpoint: &externalapi.DomainOutpoint{
			...
			unsignedTxWithLargeInputAmount, err := libnirvanawallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
				[]*libnirvanawallet.Payment{{
					Address: address,
					Amount:  10,
			...
			signedTxWithLargeInputAmount, err := libnirvanawallet.Sign(params, mnemonics, unsignedTxWithLargeInputAmount, false, version)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			txWithLargeInputAmount, err := libnirvanawallet.ExtractTransaction(signedTxWithLargeInputAmount, false)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
			...
			selectedUTXOsForTxWithLargeInputAndOutputAmount := []*libnirvanawallet.UTXO{
				{
					Outpoint: &externalapi.DomainOutpoint{
			...
			unsignedTxWithLargeInputAndOutputAmount, err := libnirvanawallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
				[]*libnirvanawallet.Payment{{
					Address: address,
					Amount:  22e6 * constants.SompiPerNirvana,
				}}, selectedUTXOsForTxWithLargeInputAndOutputAmount)
			if err != nil {
				t.Fatalf("CreateUnsignedTransactions: %+v", err)
			}

			signedTxWithLargeInputAndOutputAmount, err := libnirvanawallet.Sign(params, mnemonics, unsignedTxWithLargeInputAndOutputAmount, false, version)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			txWithLargeInputAndOutputAmount, err := libnirvanawallet.ExtractTransaction(signedTxWithLargeInputAndOutputAmount, false)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
