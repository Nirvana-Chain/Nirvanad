...
func send(conf *sendConfig) error {
	...
	if !conf.IsSendAll {
		sendAmountSompi, err = utils.KlsToSompi(conf.SendAmount)
	...
	for i, unsignedTransaction := range createUnsignedTransactionsResponse.UnsignedTransactions {
		signedTransaction, err := libnirvanawallet.Sign(conf.NetParams(), mnemonics, unsignedTransaction, keysFile.ECDSA, keysFile.Version)
		if err != nil {
			return err
		}
