...
import (
	...
	"github.com/Nirvana-Chain/nirvanad/cmd/nirvanawallet/libnirvanawallet/bip32"
...
func create(conf *createConfig) error {
	...
	fmt.Printf("Notice the above is neither a secret key to your wallet " +
		"(use \"nirvanawallet dump-unencrypted-data\" to see a secret seed phrase) " +
		"nor a wallet public address (use \"nirvanawallet new-address\" to create and see one)\n\n")
	...
	if len(signerExtendedPublicKeys) > 0 {
		cosignerIndex, err = libnirvanawallet.MinimumCosignerIndex(signerExtendedPublicKeys, extendedPublicKeys)
		if err != nil {
			return err
		}
	}
