...
type server struct {
	pb.UnimplementedNirvanawalletdServer
...
// Start starts the nirvanawalletd server
func Start(params *dagconfig.Params, listen, rpcServer string, keysFilePath string, profile string, timeout uint32) error {
	...
	if keysFile.Version == 1 {
		...
		log.Infof("new one and transfer NVA to it.")
		log.Infof("---")
	}
	...
	pb.RegisterNirvanawalletdServer(grpcServer, serverInstance)
