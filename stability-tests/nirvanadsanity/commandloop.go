...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("nirvanadsanity-nirvanad-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("nirvanad", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "NIRVANAD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "NIRVANAD-STDERR")
