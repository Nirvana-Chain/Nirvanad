...
import (
	...
	"github.com/Nirvana-Chain/nirvanad/domain/consensus/utils/mining"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("nirvanad-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("nirvanad datadir: %s", dataDir)

	nirvanadCmd, err := common.StartCmd("NIRVANAD",
		"nirvanad",
	...
	spawn("startNode-nirvanadCmd.Wait", func() {
		err := nirvanadCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("nirvanadCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes nirvanad closes ungracefully
				log.Errorf("nirvanadCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(nirvanadCmd, "nirvanadCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	nirvanaminerCmd, err := common.StartCmd("MINER",
		"nirvanaminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("nirvana-miner-Cmd.Wait", func() {
		err := nirvanaminerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(nirvanaminerCmd, "nirvanaminerCmd")
	return nil
}
