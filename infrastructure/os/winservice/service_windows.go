...
// long-running nirvanadMain (which is the real meat of nirvanad), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start nirvanadMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once nirvanad is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("nirvanadMain-windows", func() {
...
// logServiceStart logs information about nirvanad when the main server has
