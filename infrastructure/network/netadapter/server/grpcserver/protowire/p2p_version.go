...
import (
	...
	"github.com/Nirvana-Chain/nirvanad/util/mstime"
	"github.com/pkg/errors"
)

func (x *NirvanadMessage_Version) toAppMessage() (appmessage.Message, error) {
	if x == nil {
		return nil, errors.Wrapf(errorNil, "NirvanadMessage_Version is nil")
	}
...
func (x *VersionMessage) toAppMessage() (appmessage.Message, error) {
	...
	//  Full nirvana nodes set SubnetworkId==nil
	if err != nil && !errors.Is(err, errorNil) {
		return nil, err
	}
...
func (x *NirvanadMessage_Version) fromAppMessage(msgVersion *appmessage.MsgVersion) error {
