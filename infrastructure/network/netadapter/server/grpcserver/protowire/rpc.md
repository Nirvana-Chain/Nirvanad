...
NirvanadMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a NirvanadMessage) the
RPC server will respond with a ResponseMessage (likewise wrapped in a
NirvanadMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network nirvanad is
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which nirvana address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether nirvanad thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when nirvanad is not synced. That is because when nirvanad isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known nirvanad
...
p2p peers currently connected to this nirvanad.
...
| isOutbound | [bool](#bool) |  | Whether this nirvanad initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this nirvanad |
...
AddPeerRequestMessage adds a peer to nirvanad&#39;s outgoing
connection list. This will, in most cases, result in nirvanad
...
lowHash up to this nirvanad&#39;s current virtual.
...
this nirvanad. Note that this number may decrease as pruning occurs.
...
current state of this nirvanad&#39;s DAG.
...
ShutDownRequestMessage shuts down this nirvanad.
...
This call is only available when this nirvanad was started with
`--utxoindex`.
