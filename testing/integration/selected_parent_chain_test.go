...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of nirvanad instances
	nirvanad1, nirvanad2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := nirvanad1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In nirvanad2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, nirvanad2)
	}

	// Connect the two nirvanads. This should trigger sync
	// between the two nodes
	connect(t, nirvanad1, nirvanad2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := nirvanad1.rpcClient.GetVirtualSelectedParentChainFromBlock(
